...
// example url: http://localhost:8080/?avatarUrl=avatarUrlHere&username=usernameOnImage&tag=tagOnImage&number=312
app.get('*', async function (req, res) {
    ...
    try {
        ...
        if(req.query.number){
            ...
            context.fillText(`Dołączyłxś jako osoba nr ${req.query.number}`, (properties.width - 10) / 2, properties.height / 1.2 + 40)
        }
        ...
        res.send({ "buffer": canvas.toBuffer(), "image": "hewwo.png" });
        // fs.writeFileSync('hewwo.png', canvas.toBuffer())
    } catch (err) {
        res.send({ "error": true })
        console.log(err)
    }
